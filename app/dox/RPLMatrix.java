...
// File     : RPLMatrix.java
// Author   : jpl
// Created  : 01/10/11 23:28
// Modified : 04/10/11 11:29
//******************************************************************************
package replicant;
 ...
 * @version 1.1
 */
public class RPLMatrix implements Serializable
{
   ...
   /** Constructeur RPLMatrix
    * @param
    * int i - ligne
    * int j - colonne
    */
   public RPLMatrix(int i, int j)
   {
      this.setLength(i, j);
   }
   ...
   public RPLMatrix getMatriceInverse()
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
      RPLMatrix tmp = null;
      ...
      // on transpose la matrice les coeffcients seront positionnés de façon incorrect
   ...
   private RPLMatrix getNewMatrice(int row, int columns)
   {
      RPLMatrix a = new RPLMatrix(this.getRows() - 1, this.getColumns() - 1);
      int k = -1, m = 0;
      for (int i = 0; i < this.getRows(); i++)
      {
         k++;
   ...
   public RPLMatrix getMatriceTranspose()
   {
      RPLMatrix a = new RPLMatrix(this.getColumns(), this.getRows());
      long tmp = 0;
   ...
   public RPLMatrix getMatriceDistanceGraphe()
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
      int n = 1;
      for (int i = 0; i < this.getRows(); i++)
   ...
   public RPLMatrix getMatriceCompagnon()
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
   ...
   public RPLMatrix getMatriceDiametre()
   {
      int n = 1;
      RPLMatrix ai = this.sumMatrice(this.getMatriceIdentity());
      for (int i = 0; i < this.getRows(); i++)
      {
         for (int j = 0; j < this.getColumns(); j++)
         {
   ...
   public int[] getSommetPLusDistant()
   {
      ...
      RPLMatrix m = this.matricePow(this.getDiametre() - 1);
      byte n = 0;
   ...
   public RPLMatrix multiply(final RPLMatrix matrice)
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
      int k, i, j, m;
      long value = 0;
   ...
   public RPLMatrix sumMatrice(final RPLMatrix matrice)
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
      for (int i = 0; i < this.getRows(); i++)
      {
   ...
   public RPLMatrix matricePow(int n)
   {
      RPLMatrix a = this;
      RPLMatrix b = a;
      for (int i = 0; i < n - 1; i++)
   ...
   public RPLMatrix soustraction(final RPLMatrix matrice)
   {
      RPLMatrix a = new RPLMatrix(this.getRows(), this.getColumns());
      for (int i = 0; i < this.getRows(); i++)
      {
   ...
   public RPLMatrix multiplyByK(long k)
   {
      RPLMatrix a = this;
      for (int i = 0; i < this.getRows(); i++)
      {
   ...
   public RPLMatrix divByK(long k)
   {
      RPLMatrix a = this;
      for (int i = 0; i < this.getRows(); i++)
      {
   ...
   public boolean equals(RPLMatrix matrice)
   {
      for (int i = 0; i < this.getRows(); i++)
      {
         for (int j = 0; j < this.getColumns(); j++)
